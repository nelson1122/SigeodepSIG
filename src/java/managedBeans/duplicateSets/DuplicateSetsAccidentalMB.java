...
public class DuplicateSetsAccidentalMB implements Serializable {
    ...
    public void loadDuplicatedRecords() {
        ...
        if (selectedRowDuplicatedTable != null) {
            try {
                ...
                RowDataTable newRow = connectionJdbcMB.loadFatalInjuryAccidentRecord(selectedRowDuplicatedTable.getColumn1());
                if (newRow == null) {
                    ...
                    while (resultSetCount.next()) {//cargo los posibles duplicados 
                        cont++;
                        rowDataTableList.add(connectionJdbcMB.loadFatalInjuryAccidentRecord(resultSetCount.getString("victim_id")));
                    }
                    if (cont == 0) {
    ...
    public void loadValues(RowDataTable[] selectedRowsDataTableTags) {
        ...
        for (int i = 0; i < selectedRowsDataTableTags.length; i++) {
            ...
            tuplesNumber = tuplesNumber + fatalInjuryAccidentFacade.countFromTag(tagsList.get(i).getTagId());
        }
            ...
            while (resultSetFileData.next()) {
                ...
                if (!found) {//el elemento no ha sido evaluado ni adicionado
                    sql = ""
                            ...
                            + "    t1.victim_id \n"
                            + " FROM "
                            + "    duplicate t1, duplicate t2 \n"
                            + " WHERE \n"
                            + "    t2.victim_id = " + resultSetFileData.getString("victim_id") + " \n"
                            + "    AND t1.victim_id != t2.victim_id \n"
                            + "    AND levenshtein(t1.victim_nid, t2.victim_nid) < 4 \n"
                            + "    AND levenshtein(t1.victim_name, t2.victim_name) < 4 \n";
                    ResultSet resultSetCount = connectionJdbcMB.consult(sql);
                    first = true;
                    countRegisters = 0;
    ...
    public void deleteRegistry() {
        if (selectedRowDataTable != null) {
            FatalInjuryAccident fatalInjuryAccidentSelect = fatalInjuryAccidentFacade.find(Integer.parseInt(selectedRowDataTable.getColumn1()));
            if (fatalInjuryAccidentSelect != null) {
                FatalInjuries auxFatalInjuries = fatalInjuryAccidentSelect.getFatalInjuries();
                Victims auxVictims = fatalInjuryAccidentSelect.getFatalInjuries().getVictimId();
                fatalInjuryAccidentFacade.remove(fatalInjuryAccidentSelect);
