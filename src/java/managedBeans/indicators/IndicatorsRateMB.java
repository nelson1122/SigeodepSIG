 ...
 * population group in a reference period to a determined type of injury. The
 * population is taken from the number of inhabitants in a spatial
 * disaggregation as zone,commune or neighborhood.
 *
 * @author SANTOS
 */
...
public class IndicatorsRateMB {
    ...
    private List<String> listOfAvailableVariablesNames = new ArrayList<>();//lista de nombres de variables disponibles que sepueden cruzar(se visualizan en pagina)
            ...
            switch (VariablesEnum.convert(variablesCrossData.get(i).getGeneric_table())) {//nombre de variable 
                ...
                case quadrants://CUADRANTE -----------------------
                    ...
                    sqlPopulate = ""
                            ...
                            + "    WHEN " + currentIndicator.getInjuryType() + ".quadrant_id is null THEN 'SIN DATO' \n\r"
                            + "    ELSE \n\r"
                            + "      CAST((SELECT population FROM quadrants WHERE quadrant_id = " + currentIndicator.getInjuryType() + ".quadrant_id) as text) \n\r"
                            + " END AS poblacion ";
                    break;
        ...
        if (!variablesCrossData.isEmpty()) {
            try {
                ...
                if (currentTypeGraph.compareTo("barras") == 0) {
                    chart = ChartFactory.createBarChart(indicatorName, categoryAxixLabel, "Tasa", dataset, PlotOrientation.VERTICAL, true, true, false);
                ...
                if (currentTypeGraph.compareTo("lineas") == 0) {
                    chart = ChartFactory.createLineChart(indicatorName, categoryAxixLabel, "Tasa", dataset, PlotOrientation.VERTICAL, true, true, false);
                    CategoryPlot plot = (CategoryPlot) chart.getPlot();
                ...
                if (currentTypeGraph.compareTo("barras apiladas") == 0) {
                    chart = ChartFactory.createStackedBarChart(indicatorName, categoryAxixLabel, "Tasa", dataset, PlotOrientation.VERTICAL, true, true, false);
                    CategoryPlot plot = (CategoryPlot) chart.getPlot();
                    ((BarRenderer) plot.getRenderer()).setBarPainter(new StandardBarPainter());//quitar gradiente
                }
                if (currentTypeGraph.compareTo("areas") == 0) {
                    chart = ChartFactory.createAreaChart(indicatorName, categoryAxixLabel, "Tasa", dataset, PlotOrientation.VERTICAL, true, true, false);
...
//                //}
    ...
    public void reset() {
        ...
        chartImage = null;
        currentVariableConfiguring = null;
        listOfCrossVariablesNames = new ArrayList<>();
...
//                valuesConf.add("VIOLENCIA INTRAFAMILIAR");
     ...
     * that contains the result of the cross of variables
     */
